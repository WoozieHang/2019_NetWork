...
void InitialRouter(){
	RouterTable[0].ip=(192<<24)+(168<<16)+(174<<8)+0;
	RouterTable[0].next_hop=(192<<24)+(168<<16)+(230<<8)+132;
	strcpy(RouterTable[0].interface,"ens38");
	RouterTable[0].netmask=0xffffff00;
	router_num++;

	RouterTable[1].ip=(192<<24)+(168<<16)+(211<<8)+0;
	RouterTable[1].next_hop=DEFAULT_IP;//means *
...
void InitialVpn(){
	...
	VpnTable[0].ip=(192<<24)+(168<<16)+(114<<8)+0;
	VpnTable[0].vpn_server=(192<<24)+(168<<16)+(174<<8)+130;
...
void InitialArp(){
	ArpTable[0].ip=(192<<24)+(168<<16)+(230<<8)+132;
	InitialMac(&(ArpTable[0].mac_addr),0x00,0x0c,0x29,0x2a,0x52,0xa3);
	arp_num++;

	ArpTable[1].ip=(192<<24)+(168<<16)+(211<<8)+130;
	InitialMac(&(ArpTable[1].mac_addr),0x00,0x0c,0x29,0x5d,0x92,0xda);
	arp_num++;
}
...
int AnalyseEth(char* eth_head,int* interface_num,MacAddr* src_addr){
		...
		if(p[0]==0x00&&p[1]==0x0c&&p[2]==0x29&&p[3]==0x05&&p[4]==0x0f&&p[5]==0x71){
			*interface_num=0;
			printf("<start>\n===========================================================\n");
			printf("***********************************************************\n");
			printf("an ether frame from interface ens33\n");
		}
		else if(p[0]==0x00&&p[1]==0x0c&&p[2]==0x29&&p[3]==0x05&&p[4]==0x0f&&p[5]==0x7b){
			*interface_num=1;
			printf("<start>\n===========================================================\n");
			printf("***********************************************************\n");
...
void Repacked(uint32_t vpn_server,uint8_t* ip_head){
	...
	//use ipv4 and set headlen
        ...
        ipHeader->ttl=255;
	//src addr
	ipHeader->src_ip=htonl((192<<24)+(168<<16)+(230<<8)+131);
        //destination address
        ipHeader->dest_ip =htonl((192<<24)+(168<<16)+(174<<8)+130);
	//check sum
