...
void InitialRouter(){
	RouterTable[0].ip=(192<<24)+(168<<16)+(114<<8)+0;
	RouterTable[0].next_hop=DEFAULT_IP;
	strcpy(RouterTable[0].interface,"ens38");
	RouterTable[0].netmask=0xffffff00;
	router_num++;

	RouterTable[1].ip=(192<<24)+(168<<16)+(230<<8)+0;
	RouterTable[1].next_hop=(192<<24)+(168<<16)+(174<<8)+128;
	strcpy(RouterTable[1].interface,"ens33");
	RouterTable[1].netmask=0xffffff00;
	router_num++;
}
...
int SearchRouter(uint32_t ip){
	for(int i=0;i<router_num;i++){
		...
		//printf("ip&(RouterTable[%d].netmask):%x\n\n",i,ip&(RouterTable[i].netmask));
		if((RouterTable[i].ip)==(ip&(RouterTable[i].netmask))){	
			return i;
		}
	}
...
int vpn_num=0;
void InitialVpn(){
	VpnTable[0].ip=(192<<24)+(168<<16)+(211<<8)+0;
	VpnTable[0].vpn_server=(192<<24)+(168<<16)+(230<<8)+131;
	VpnTable[0].netmask=0xffffff00;
	vpn_num++;

}
int SearchVpn(uint32_t ip){
	for(int i=0;i<vpn_num;i++){
		//printf("router table[%d].ip:%x\n",i,RouterTable[i].ip);
		//printf("ip&(RouterTable[%d].netmask):%x\n\n",i,ip&(RouterTable[i].netmask));
		if((VpnTable[i].ip)==(ip&(VpnTable[i].netmask))){	
			return i;
		}
	}
	//printf("ip:%d.%d.%d.%d\n",(ip>>24)&0xff,(ip>>16)&0xff,(ip>>8)&0xff,(ip)&0xff);	
...
int SearchServer(uint32_t server){
	for(int i=0;i<vpn_num;i++){
		//printf("router table[%d].ip:%x\n",i,RouterTable[i].ip);
		//printf("ip&(RouterTable[%d].netmask):%x\n\n",i,ip&(RouterTable[i].netmask));
		if(VpnTable[i].vpn_server==server){	
			return i;
		}
	}
	//printf("ip:%d.%d.%d.%d\n",(ip>>24)&0xff,(ip>>16)&0xff,(ip>>8)&0xff,(ip)&0xff);	
...
void InitialArp(){
	ArpTable[0].ip=(192<<24)+(168<<16)+(114<<8)+130;
	InitialMac(&(ArpTable[0].mac_addr),0x00,0x0c,0x29,0x25,0xa7,0x14);
	arp_num++;

	ArpTable[1].ip=(192<<24)+(168<<16)+(174<<8)+128;
	InitialMac(&(ArpTable[1].mac_addr),0x00,0x0c,0x29,0x2a,0x52,0xad);
	arp_num++;
}
...
int AnalyseEth(char* eth_head,int* interface_num,MacAddr* src_addr){
		...
		if(p[0]==0x00&&p[1]==0x0c&&p[2]==0x29&&p[3]==0xdb&&p[4]==0x06b&&p[5]==0x7c){
			*interface_num=0;
			printf("<start>\n===========================================================\n");
			printf("***********************************************************\n");
			printf("an ether frame from interface ens33\n");
		}
		else if(p[0]==0x00&&p[1]==0x0c&&p[2]==0x29&&p[3]==0xdb&&p[4]==0x06b&&p[5]==0x86){
			*interface_num=1;
			printf("<start>\n===========================================================\n");
			printf("***********************************************************\n");
			printf("an ether frame from interface ens38\n");
		}
		else if(p[0]==0xff&&p[1]==0xff&&p[2]==0xff&&p[3]==0xff&&p[4]==0xff&&p[5]==0xff){
			*interface_num=2;
			printf("<start>\n===========================================================\n");
			printf("***********************************************************\n");
			printf("an enther frame broadcast\n");
		}
		else {
			*interface_num=-1;
			return -1;
...
void Repacked(uint32_t vpn_server,uint8_t* ip_head){
	

	for(int i=83;i>=0;i--)
	ip_head[i+20]=ip_head[i];
	
	
	

	IP_HEADER* ipHeader=(IP_HEADER*)(ip_head);
 	IP_HEADER* copy_ipHeader=(IP_HEADER*)(&ip_head[20]);
	//use ipv4 and set headlen
         ipHeader->head_version_len =copy_ipHeader->head_version_len;
        //service type
        ipHeader->tos = copy_ipHeader->tos ;
        ipHeader->total_len = htons(0x68);
        ipHeader->id=copy_ipHeader->id;
        //zero flag tag
        ipHeader->flags=copy_ipHeader->flags;
        //use icmp protocol
        ipHeader->proto=IPPROTO_IPIP;
        //time to live
        ipHeader->ttl=255;
	//src addr
	ipHeader->src_ip=htonl((192<<24)+(168<<16)+(174<<8)+130);
        //destination address
        ipHeader->dest_ip =htonl((192<<24)+(168<<16)+(230<<8)+131);
	//check sum
	ipHeader->check_sum=0;
	ipHeader->check_sum = htons(check_sum( (unsigned short *)ipHeader,sizeof(IP_HEADER) +sizeof(IP_HEADER)+ sizeof(ICMP_HEADER)+56)); 
...
void unpacked(uint8_t* ip_head){
	

	for(int i=0;i<84;i++)
	ip_head[i]=ip_head[i+20];
...
void Forwarding(char* interface,MacAddr dest_mac,char* buffer){
	int  frame_length, sd;
	uint8_t src_mac[6];
	...
	//use ipv4 and set headlen
        ...
        //time to live
        ipHeader->ttl=copy_ipHeader->ttl-1;
	//src addr
	ipHeader->src_ip=copy_ipHeader->src_ip;
        //destination address
        ipHeader->dest_ip =copy_ipHeader->dest_ip;
	
	
	//fill ipip header
	ipHeader=(IP_HEADER*)(&ether_frame[34]);
       	copy_ipHeader=(IP_HEADER*)(&buffer[34]);
	//use ipv4 and set headlen
         ipHeader->head_version_len =copy_ipHeader->head_version_len;
        //service type
        ipHeader->tos = copy_ipHeader->tos ;
	...
	ICMP_HEADER* copy_icmpHeader=(ICMP_HEADER*)(copy_ipHeader+1);
    	icmpHeader->type = copy_icmpHeader->type;
	...
	//cpdata
	char* data=(char*)(icmpHeader+1);
	char* copy_data=(char*)(copy_icmpHeader+1);
	for(int i=0;i<56;i++)
		data[i]=copy_data[i];
	...
	//check sum
	ipHeader->check_sum=0;
	ipHeader->check_sum = check_sum( (unsigned short *)ipHeader,sizeof(IP_HEADER) + sizeof(ICMP_HEADER)+56); 
	
	//fill ip header
	ipHeader=(IP_HEADER*)(&ether_frame[14]);
	
	//check sum
	ipHeader->check_sum=0;
	ipHeader->check_sum = check_sum( (unsigned short *)ipHeader,sizeof(IP_HEADER)+sizeof(IP_HEADER) + sizeof(ICMP_HEADER)+56); 
...
void Forwarding2(char* interface,MacAddr dest_mac,char* buffer){
	int  frame_length, sd;
	uint8_t src_mac[6];
	uint8_t ether_frame[IP_MAXPACKET];
	struct sockaddr_ll device;
...
int main(int argc,char* argv[]){
	...
	while(1){
		...
		if(proto_choice==1){
			...
			//search the ip table to find the next hop
			printf("dst_ip:%x ",dst_ip);
			uint32_t next_index;
			if(interface_num==2)
				continue;
			else if((next_index=SearchRouter(dst_ip))==-1){
				printf("check the router table!\n");
				return 0;
			}
	//search the arp table to get the mac addr and this stage we initial arp table instead
			MacAddr next_mac;
			if(RouterTable[next_index].next_hop!=DEFAULT_IP){
				if(SearchArp(RouterTable[next_index].next_hop,&next_mac)==0){
					printf("check the arp table\n");
					return 0;			
				}
			}
			else {
				if(SearchArp(dst_ip,&next_mac)==0){
				printf("check the arp table\n");
				return 0;			
				}
			}

			/*printf("next_mac:%x:%x:%x:%x:",next_mac.mac[0],next_mac.mac[1],next_mac.mac[2],next_mac.mac[3]);
			printf("%x:%x\n",next_mac.mac[4],next_mac.mac[5]);
			printf("next_ip:%d.%d.%d.%d\n",(RouterTable[next_index].next_hop>>24)&0xff,(RouterTable[next_index].next_hop>>16)&0xff,(RouterTable[next_index].next_hop>>8)&0xff,RouterTable[next_index].next_hop&0xff);*/
			if(interface_num==1)
