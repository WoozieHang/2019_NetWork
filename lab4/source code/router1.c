...
void InitialArp(){
	...
	ArpTable[1].ip=(192<<24)+(168<<16)+(2<<8)+1;
	InitialMac(&(ArpTable[1].mac_addr),0x00,0x0c,0x29,0x68,0x60,0xfb);
	arp_num++;
}
...
int AnalyseEth(char* eth_head,int* interface_num,MacAddr* src_addr){
		...
		if(p[0]==0x00&&p[1]==0x0c&&p[2]==0x29&&p[3]==0x5d&&p[4]==0x92&&p[5]==0xda){
			*interface_num=0;
			printf("<start>\n===========================================================\n");
			printf("***********************************************************\n");
			printf("an ether frame from interface ens33\n");
		}
		else if(p[0]==0x00&&p[1]==0x0c&&p[2]==0x29&&p[3]==0x5d&&p[4]==0x92&&p[5]==0xe4){
			*interface_num=1;
			printf("<start>\n===========================================================\n");
			printf("***********************************************************\n");
