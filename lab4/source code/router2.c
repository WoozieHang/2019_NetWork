...
void InitialRouter(){
	RouterTable[0].ip=(192<<24)+(168<<16)+(4<<8)+0;
	RouterTable[0].next_hop=DEFAULT_IP;//means *
	strcpy(RouterTable[0].interface,"ens39");
	RouterTable[0].netmask=0xffffff00;
	router_num++;
	
	RouterTable[1].ip=(192<<24)+(168<<16)+(2<<8)+0;
	RouterTable[1].next_hop=(192<<24)+(168<<16)+(3<<8)+1;
	strcpy(RouterTable[1].interface,"ens38");
...
void InitialArp(){
	ArpTable[0].ip=(192<<24)+(168<<16)+(4<<8)+2;
	InitialMac(&(ArpTable[0].mac_addr),0x00,0x0c,0x29,0x05,0xf6,0x08);
	arp_num++;
	
	ArpTable[1].ip=(192<<24)+(168<<16)+(3<<8)+1;
	InitialMac(&(ArpTable[1].mac_addr),0x00,0x0c,0x29,0x5d,0x92,0xe4);
	arp_num++;
}
...
int AnalyseEth(char* eth_head,int* interface_num,MacAddr* src_addr){
		...
		if(p[0]==0x00&&p[1]==0x0c&&p[2]==0x29&&p[3]==0x3f&&p[4]==0x68&&p[5]==0xe4){
			*interface_num=0;
			printf("<start>\n===========================================================\n");
			printf("***********************************************************\n");
			printf("an ether frame from interface ens38\n");
		}
		else if(p[0]==0x00&&p[1]==0x0c&&p[2]==0x29&&p[3]==0x3f&&p[4]==0x68&&p[5]==0xee){
			*interface_num=1;
			printf("<start>\n===========================================================\n");			
			printf("***********************************************************\n");
